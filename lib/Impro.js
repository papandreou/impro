...
var sharp;
var Gifsicle;
var isOperationByEngineNameAndName = {};
...
if (gm) {
    ...
    ['jpeg', 'png', 'gif'].forEach(function (formatName) {
...
function Impro(options) {
    ...
    this.defaultEngineName = options.defaultEngineName || (sharp && 'sharp') || 'gm';
...
Impro.prototype.checkSharpOrGmOperation = function (operation) {
    if (operation.name === 'resize' && typeof this.maxOutputPixels === 'number' && operation.args.length >= 2 && operation.args[0] * operation.args[1] > this.maxOutputPixels) {
...
Pipeline.prototype._freeze = function () {
    if (!this._frozen) {
        ...
        this._streams[this._streams.length - 1].once('readable', function () {
            console.log("last stream now readable");
        }.bind(this));
...
Pipeline.prototype._flush = function () {
    if (this.queue.length > 0) {
        ...
        if (engineName === 'gifsicle') {
            ...
            this.add(sharpOperationsForThisInstance.reduce(function (sharpInstance, operation) {
                this.impro.checkSharpOrGmOperation(operation);
            ...
            var gmOperationsForThisInstance = [].concat(this.queue);
...
Pipeline.prototype.add = function (options) {
    ...
    if (options && typeof options.pipe === 'function') {
        ...
        var filter;
        if (this.impro.filters[operationName]) {
            this._flush();
            ...
            if (filter) {
                ...
                if (filter.outputContentType) {
                    targetContentType = filter.outputContentType;
                }
            }
            ...
            if (candidateEngineNames.length > 0) {
                if (this.currentEngineName && !isOperationByEngineNameAndName[this.currentEngineName]) {
                    this._flush();
                }
...
Object.keys(isOperationByEngineNameAndName).forEach(function (engineName) {
    Object.keys(isOperationByEngineNameAndName[engineName]).forEach(function (operationName) {
        ...
        Impro.prototype[operationName] = Impro.prototype[operationName] || function () {
            return this.pipeline(this, {name: operationName, args: Array.prototype.slice.call(arguments)});
        };
    });
});
