...
module.exports = {
    ...
    validateOperation: function (name, args) {
        switch (name) {
        case 'copy':
            return args.length === 1 && (args[1] === 'none' || args[1] === 'comments' || args[1] === 'all');
        ...
        case 'flip':
            return args.length === 1 && (args[0] === 'horizontal' || args[0] === 'vertical');
        case 'rotate':
            return args.length === 1 && (args[0] === 90 || args[0] === 180 || args[0] === 270);
        case 'maxmemory':
        case 'restart':
            return args.length === 1 && typeof args[0] === 'number' && args[0] > 0 && args[0] === Math.floor(args[0]);
        }
    },
    execute: function (pipeline, operations, options) {
        ...
        operations.forEach(({name, args}) => {
            if (name === 'extract') {
                commandLineArgs.push('-crop', args[2] + 'x' + args[3] + '+' + args[0] + '+' + args[1]);
            } else if (name === 'crop') {
                commandLineArgs.push('-crop', args[0] + 'x' + args[1] + '+' + args[2] + '+' + args[3]);
            } else {
